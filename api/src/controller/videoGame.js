...
module.exports.deleteVideoGame = async (req, res) => {
  ...
  try {
    ...
    const { rows : publicationIds } = await PublicationModel.getPublicationId(client, id);
    if (publicationIds.length !== 0) {
        for (const publicationId of publicationIds) {
            await GameController.deletePublicationGames(client, publicationId.id);
            //! todo : delete the publication from the await function for cédric please dépeche you
