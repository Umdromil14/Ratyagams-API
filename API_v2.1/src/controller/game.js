...
async function getGame(userId, publicationId, res) {
    try {
        ({ userId, publicationId } = validateObject({ userId, publicationId }, gameIdsSchema.partial()));
    } catch (error) {
    ...
    try {
        const { rows: games } = await GameModel.getGames(client, userId, publicationId);
        if (games.length === 0) {
...
/**
 * Get all games, or all games by user id,
 * or all games by publication id,
 * or a game by user id and publication id;
...
/**
 * Get all session user's games or a game by publication id;
...
async function updateGame(userId, publicationId, updateValues, res) {
    try {
        ({ userId, publicationId } = validateObject({ userId, publicationId }, gameIdsSchema));
    ...
    const client = await pool.connect();
    try {
        const { rowCount } = await GameModel.updateGame(client, userId, publicationId, updateValues);
        if (rowCount) {
...
module.exports.updateGameFromAdmin = async (req, res) => {
    await updateGame(req.params.userId, req.params.publicationId, req.body, res);
}
