...
const { publicationToGetSchema } = require("../zod/schema/publication");
...
module.exports.getPublication = async (req, res) => {
    ...
    try {
        ({ publicationId, videoGameId, videoGameName, platformCode } = validateObject(req.query, publicationToGetSchema));
    } catch (error) {
    ...
    const client = await pool.connect();
    try {
        const { rows: publications } = await PublicationModel.getPublication(client, publicationId, platformCode, videoGameId, videoGameName);
