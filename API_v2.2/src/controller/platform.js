...
module.exports.addPlatform = async (req, res) => {
    ...
    try {
        ({ code, description, abbreviation } = validateObject(req.body, platformSchema));
    } catch (error) {
    ...
    const client = await pool.connect();
    try {
        await PlatformModel.createPlatform(client, code, description, abbreviation);
...
module.exports.addPlatformWithNewVideoGames = async (req, res) => {
    ...
    try {
        ({ code, description, abbreviation, video_games: videoGames } = validateObject(req.body, platformWithVideoGamesSchema));
    } catch (error) {
        res.status(HTTPStatus.BAD_REQUEST).send(error.message);
        return;
    }
