...
module.exports.getVideoGame = async (req, res) => {
	// TODO refaire comme publication
	const {z} = require("zod");

	const videoGameToGetSchema = z.object({
		id : z.coerce.number().int().optional(),
		name : z.string().optional()
	});

	const result = videoGameToGetSchema.safeParse(req.query);
    if (!result.success) {
	...
	const client = await pool.connect();
	try {
		const { rows: videoGames } = await VideoGameModel.getVideoGames(client, id, name);
		if (videoGames.length === 0) {
			res.status(HTTPStatus.NOT_FOUND).send("No video game found");
		} else {
			res.json(videoGames);
