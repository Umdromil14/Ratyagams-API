...
const { loginSchema , userSchema, userSchemaWithGames, updateMyAccountSchema } = require("../schema/user.js");
...
module.exports.login = async (req, res) => {
    ...
    try {
        let user;
        if (login.contains("@")) {
            user = await UserDB.getUserFromEmail(client, login);
        } else {
            user = await UserDB.getUserFromUsername(client, login);
        }
...
module.exports.postUser = async (req, res) => {
    ...
    const { username, email, firstname, lastname, password, is_admin: isAdmin } = result.data;

    const client = await pool.connect();

    try {

        //! faire passer dans UNIQUE_VIOLATION ?
...
module.exports.deleteMyAccount = async (req, res) => {
    ...
    // TODO blacklist token with setTimeout
};
...
module.exports.updateMyAccount = async (req, res) => {
    ...
    try {
        const { id, isAdmin } = req.session.data;
        const user = (await UserDB.getUserFromId(client, id)).rows[0];
