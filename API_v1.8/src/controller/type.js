...
module.exports.createType = async (req, res) => {
    ...
    try {
        if(!(await TypeModel.typeExistsName(client, name))){
...
module.exports.updateType = async (req, res) => {
    ...
    try {
        if(isNaN(id)){
            ...
            if(! await TypeModel.typeExistsName(client, newName)){
                const {rowCount} = await TypeModel.updateType(client, id, updateValues);
                if(rowCount){
