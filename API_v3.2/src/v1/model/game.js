 ...
 * @param {boolean=} isOwned whether the user owns the game; defaults to `false`
 * @param {number=} reviewRating the rating the user gave to the game; defaults to `null`
 * @param {string=} reviewComment the comment the user gave to the game; defaults to `null`
 * @param {Date=} reviewDate the date the user gave the review; defaults to `null`
 * 
 ...
 * @param {number=} videoGameId the id of the video game; if specified, only games from this video game will be returned
...
module.exports.getGames = async (
    ...
    if (videoGameId !== undefined) {
        queryConditions.push(
            `publication_id in (SELECT id FROM publication WHERE video_game_id = $${
    ...
    query += ` ORDER BY user_id, publication_id ASC`;
...
module.exports.deleteGamesFromUser = async (client, userId) => {
    return await client.query(`DELETE FROM game WHERE user_id = $1`, [userId]);
};
...
module.exports.deleteGamesFromPublication = async (client, publicationId) => {
    return await client.query(`DELETE FROM game WHERE publication_id = $1`, [
        publicationId,
    ]);
};
 ...
 * @throws {Error} if the request fails
 *
 * @returns {Promise<boolean>} a promise that contains a boolean; `true` if the game exists, `false` otherwise
 */
